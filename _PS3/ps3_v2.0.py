...
class factorGraph (object):
    ...
    def computeMu(self):
        for constr in self.constrFactor:
            ...
            for j in range(0, w):
                ...
                ovNode=copy.deepcopy(self.constrFactor[constr])
                ovNode.remove(vNode)
                G={}
                G[0]=1
                G[1]=1
                for i in ovNode:
                    G[0]*=(self.V[i][constr][0] - self.V[i][constr][1])
                if (constr not in self.mu):
                    self.mu[constr]={}
                if(vNode not in self.mu[constr]):
                    self.mu[constr][vNode]={}

                self.mu[constr][vNode][0]=(G[0]+G[1])/2
                self.mu[constr][vNode][1]=(G[1]-G[0])/2
        #pprint.pprint(self.mu)
    ...
    def computeCodeWord(self, inputData):
        ...
        #print ":)"
        return self.G_matrix*inputData
    ...
    def checkLBP(self,max_iterations, expectedOutput, garbledData,timeseed):
        ...
        for i in range(0, max_iterations): 
            ...
            #pprint.pprint(predictedData.T)
            #pprint.pprint(expectedOutput)

            if (self.image_data==1):
                if i in itr_to_print:
                    image_capture_itr.append(np.mat(predictedData)[:, 0:self.x*self.y])
        ...
        pprint.pprint(np.mat(predictedData))
